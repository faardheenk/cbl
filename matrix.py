    ...
    for i, matrix_item in enumerate(matrix_keys):
        try:
            ...
            if not available_cbl_cols or not available_insurer_cols:
                print(f"⚠️ Skipping matrix key - insufficient columns for key rebuilding")
                print(f"   CBL available: {available_cbl_cols}, Insurer available: {available_insurer_cols}")
                continue
            
            cols_cbl = available_cbl_cols
            cols_insurer = available_insurer_cols
            ...
            if normalized_reconstructed == normalized_original:
                ...
                for j, lhs_key in enumerate(lhs_actual_keys):
                    ...
                    cbl_amount = cbl_df.at[cbl_row_index, "ProcessedAmount_Clean"]
